...
var donutLoadStroke = (Donut => {
  d3.json(stroke).then(data => {
    ...
    // set hypertention value
    var searchHypertenstion = 1;

    // find length of thoise that have had a stroke
    var strokeLength = data.filter(count => count.stroke === searchStroke).length

    // filter through data to find title
    var strokeYes = data.filter(yes => yes.stroke === searchStroke && yes.hypertension === searchHypertenstion).length; 

    // calculate and set values for those with and without hypertension (w/ stroke)
...
var donutLoadNull = (donut => {
  d3.json(stroke).then(data => {
    ...
    var strokeNo = data.filter(no => no.stroke === searchStroke && no.hypertension === searchHypertenstion).length;

    // calculate and set values for those with and without heart disease (w/o stroke)
    var hypertenstionData = [
      {x: "Yes", value: strokeNo},
      {x: "No", value: strokeLength - strokeNo}
    ];

    // create pie chart, set data
    chart = anychart.pie(hypertenstionData);
